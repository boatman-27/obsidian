...
Clones the repo at `＜repo location＞` and applies the template from `＜template directory＞` to the newly created local branch. A thorough reference on Git templates can be found on our [git init page](https://www.atlassian.com/git/tutorials/setting-up-a-repository/git-init).  
