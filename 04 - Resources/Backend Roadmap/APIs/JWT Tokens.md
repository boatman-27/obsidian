...
General flow:
1. The issuer creates a new JWT and sets the claims (e.g., user ID, permissions).
2. The JWT is signed using a secret or a key pair.
3. The token is returned to the client and stored (usually in localStorage, sessionStorage, or a cookie).
4. The client sends the token in the Authorization header of future requests.
5. The server verifies the token and processes the request if it's valid.

---

## JWT Authentication Example

1. A user logs in by providing credentials (e.g., username and password).
2. The server validates the credentials.
3. If valid, the server issues an **access token** and often a **refresh token**.
4. The **access token** is sent back to the client and stored.
5. The client sends this token in future requests via the Authorization header.
6. When the access token expires, the client can use the **refresh token** to get a new access token without re-logging in.
---
## Access Tokens vs. Refresh Tokens
JWT-based authentication typically uses **two tokens**:
### Access Token
- **Purpose:** Grants access to protected resources.
- **Lifespan:** Short-lived (e.g., 5–15 minutes).
- **Usage:** Sent with each request to authorize access.
- **Security Risk:** If stolen, it can be used to access resources until it expires.

### Refresh Token
- **Purpose:** Used to obtain new access tokens without requiring the user to log in again.
- **Lifespan:** Long-lived (e.g., hours or days).
- **Usage:** Stored securely (e.g., HTTP-only cookie) and sent only when refreshing the access token.
- **Security Risk:** Must be stored securely. If compromised, an attacker can continually generate new access tokens.

**Note:** Refresh tokens are typically stored and managed more securely than access tokens and may include additional checks like IP address or device fingerprinting.
